...
3. Parse the JSON object from `begin_turn.result.content[].text`. Read `sessionId` and `turnToken` from that parsed `begin_turn` object and continue with `welcome_user` when the first-request bootstrap applies and with the active skill's permitted `execute_tool` operations. Usable `sessionId` and `turnToken` values are non-empty opaque strings. Only an explicit `begin_turn` error or failed tool call fails immediately; do not retry it or invent either value.

   If `begin_turn` reports success but `result.content[].text` is not valid JSON or the parsed object lacks either a usable `sessionId` or `turnToken`, retry `begin_turn` exactly once, for at most two total attempts. Repeat the same `userIntent` and `clientAttribution`. If the first parsed object contains a usable `sessionId`, include it in the retry; otherwise include the recovered conversation `sessionId` when one existed. An explicit `begin_turn` error or failed tool call fails immediately; do not retry. Do not use Bash, `sleep`, polling, delayed waiting, or tool rediscovery. If the retry still has invalid JSON or lacks either value, use the active skill's business-language availability response and do not call `welcome_user` or `execute_tool`.
