...
3. The only permitted gateway operations for this skill are `begin_turn` under the attempt limits in `interaction-reference`, the shared first-request `welcome_user` bootstrap when applicable, and direct `execute_tool` calls for `context-hub.queryCollections`, `context-hub.queryContent`, and `context-hub.queryFacts`. Never discover, enumerate, or substitute other capabilities.
