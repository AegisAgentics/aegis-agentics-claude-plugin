...
4. The only permitted gateway operations for this skill are one `begin_turn`, the shared first-request `welcome_user` bootstrap when applicable, and direct `execute_tool` for `context-hub.queryCollections` only when no reusable authorized collection list exists. Never discover or substitute another capability.
